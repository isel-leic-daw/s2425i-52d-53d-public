...
class RepositoryJdbiTests {
    ...
    @Test
    fun `test create event and find it`() = runWithHandle { handle ->
        val alice = RepositoryUserJdbi(handle).let {
            it.createUser(
                "Alice", "alice@example.com"
            )
        }
        val repoEvents = RepositoryEventJdbi(handle).also {
            it.createEvent(
                title = "Team Meeting",
    ...
    @Test
    fun `test replacing a time slot in an event`() = runWithHandle { handle ->
        val repoUsers = RepositoryUserJdbi(handle).also {
            it.createUser(
                "Alice", "alice@example.com"
            )
        }
        ...
        val repoEvents = RepositoryEventJdbi(handle).also { it.createEvent(
            title = "Team Meeting",
            description = "Discuss project updates",
