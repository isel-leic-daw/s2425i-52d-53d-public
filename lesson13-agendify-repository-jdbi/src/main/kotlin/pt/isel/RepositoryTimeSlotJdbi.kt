...
class RepositoryTimeSlotJdbi(private val handle: Handle) : RepositoryTimeSlot {

    val repoEvents = RepositoryEventJdbi(handle)
    ...
    private fun mapRowToTimeSlot(rs: ResultSet, event: Event): TimeSlotSingle {
        val owner = if (rs.getInt("owner_id") != 0) {
            User(
